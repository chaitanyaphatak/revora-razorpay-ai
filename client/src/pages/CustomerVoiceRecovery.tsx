...
export default function CustomerVoiceRecovery() {
  ...
  useEffect(() => {
    speechControllerRef.current = new BrowserSpeechController(
      ...
      (errorMsg) => {
        console.debug("[SpeechService] Info:", errorMsg);
        // Do NOT flip to text mode automatically — keep voice mic active!
        setVoiceState("idle");
      },
      (state) => {
  ...
  const handleStartListening = () => {
    ...
    if (!started) {
      // Keep voice active; let user tap again
      setVoiceState("idle");
    }
  };
              ...
              {!useTextMode ? (
                ...
                <form onSubmit={handleSendTextMessage} className="flex gap-2">
                  <Input
                    ...
                    placeholder="Type in Hinglish (e.g. Payment link do, Razorpay se pay karna hai)..."
